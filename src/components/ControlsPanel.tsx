    ...
    <div className="safe-t safe-r safe-fit fixed top-3 right-3 sm:top-4 sm:right-4 z-40 w-[calc(100vw-1.5rem)] sm:w-[410px] max-h-[calc(100dvh-1.5rem)] overflow-y-auto overscroll-contain rounded-2xl bg-neutral-950/94 backdrop-blur-2xl border border-neutral-800/90 shadow-2xl p-3.5 sm:p-4 text-xs select-none transition-all">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-xs font-semibold tracking-wider uppercase text-neutral-100 flex items-center gap-1.5">
              ...
              <span className="text-micro text-amber-400 font-mono bg-amber-950/60 px-1.5 py-0.5 rounded border border-amber-800/40">24 Топологии</span>
            </h2>
            <p className="text-micro text-neutral-400 font-mono">WebGPU / WebGL2 · Золотое Сечение φ · 60-250 FPS</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-1">
          {onOpenAtlasModal && (
            <button
              ...
              className="px-2 py-1 rounded-lg bg-indigo-500/15 border border-indigo-500/40 text-indigo-300 hover:bg-indigo-500/25 transition text-micro font-medium flex items-center gap-1 shadow-sm"
          ...
          {onSaveToFeed && (
            <button
              ...
              className="px-2 py-1 rounded-lg bg-amber-500/15 border border-amber-500/30 text-amber-300 hover:bg-amber-500/25 transition text-micro font-medium flex items-center gap-1"
      ...
      <div className="grid grid-cols-4 gap-1 bg-neutral-900/80 p-1 rounded-xl border border-neutral-800/80 mb-3.5 text-center">
        <button
          ...
          className={`py-1.5 rounded-lg text-micro font-medium transition ${
          ...
          <div className="flex flex-wrap gap-1 pb-1">
            {[
              ...
              <button
                ...
                className={`px-2 py-1 rounded-md text-micro-sm transition ${
            ...
            {filteredArchitectures.map((arch) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-micro text-neutral-400 mt-0.5 line-clamp-1">{arch.desc}</p>
                  <code className="text-micro-sm text-amber-400/90 font-mono mt-1 block">{arch.formula}</code>
                </button>
              );
            })}
          </div>
      ...
      {activeTab === 'composition' && (
        <div className="space-y-3.5">
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <span className="text-nano font-semibold text-neutral-200 block">Оператор Наложения (Algebra SDF)</span>
            <div className="grid grid-cols-2 gap-1.5">
              {ALL_COMPOSITE_OPS.map((op) => (
                <button
                  key={op}
                  onClick={() => onParamsChange(prev => ({ ...prev, compositeOp: op }))}
                  className={`p-2 rounded-lg border text-left text-micro transition flex items-center justify-between ${
                ...
                >
                  ...
                  <code className="text-micro-sm text-amber-400/80 font-mono ml-1">{COMPOSITE_OP_SYMBOLS[op]}</code>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-nano font-semibold text-neutral-200">Вторичная Геометрия (Слой 2)</span>
              <span className="text-micro text-amber-400 font-mono">{Math.round((params.hybridBlend ?? 0.35) * 100)}%</span>
            </div>
            <select
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-nano font-semibold text-neutral-200">Третичная Геометрия (Слой 3)</span>
              <span className="text-micro text-amber-400 font-mono">{Math.round((params.tertiaryBlend ?? 0.2) * 100)}%</span>
            </div>
            <select
          ...
          <div className="space-y-2 bg-neutral-900/60 p-3 rounded-xl border border-neutral-800">
            ...
            <div className="flex justify-between text-nano pt-2">
      ...
      {activeTab === 'camera' && (
        <div className="space-y-3.5">
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <span className="text-nano font-semibold text-neutral-200 block">Топологический Режим Камеры</span>
            <div className="grid grid-cols-2 gap-1.5">
              {[
                ...
                >
                  <div className="font-medium text-micro">{mode.label}</div>
                  <div className="text-micro-xs text-neutral-500">{mode.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 flex items-center justify-between">
            <div>
              <span className="text-nano font-semibold text-neutral-200 block">Автовращение</span>
              <span className="text-micro-xs text-neutral-500">Пауза при взаимодействии, возобновление через ~3с</span>
            </div>
            <button
              id="toggle-auto-rotate-btn"
              onClick={() => onParamsChange(prev => ({ ...prev, autoRotate: !prev.autoRotate }))}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-micro font-medium transition ${
                params.autoRotate
                  ? 'bg-amber-950/50 border-amber-500/70 text-amber-200'
                  : 'bg-neutral-900/80 border-neutral-800 text-neutral-400 hover:text-neutral-200'
              }`}
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <div className="flex justify-between items-center text-nano">
              <span className="font-semibold text-neutral-200">Механика Рендеринга</span>
              <span className="text-micro font-mono text-cyan-400 uppercase">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {[
                ...
                >
                  <div className="font-medium text-micro">{style.label}</div>
                  <div className="text-micro-xs text-neutral-500">{style.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-nano font-semibold text-neutral-200">Мультиосевая Томография (Slice)</span>
              <span className="text-micro font-mono text-amber-400">
            ...
            <div className="grid grid-cols-4 gap-1">
              {[
                ...
                <button
                  ...
                  className={`py-1 px-1.5 rounded text-center text-micro font-mono transition border ${
            ...
            <input
              ...
              className="w-full accent-amber-400"
            />
            <p className="text-micro-sm text-neutral-400">Вскрывает внутренние камеры со светящейся каймой выбранной нормали.</p>
          </div>

          {/* Spatial Instruments: Probe HUD & Macro Focus */}
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <span className="text-nano font-semibold text-neutral-200 block">Инструменты Исследователя</span>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div>
                  <div className="font-medium text-micro">Топологический Зонд</div>
                  <div className="text-micro-xs text-neutral-500">Прицел и телеметрия d(p)</div>
                </div>
              ...
              >
                <div>
                  <div className="font-medium text-micro">Ультра-Макро 10⁻⁴</div>
                  <div className="text-micro-xs text-neutral-500">Микрошаг реймарша</div>
                </div>
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <div className="flex justify-between items-center text-nano">
              ...
              <span className="text-micro font-mono text-amber-400">{(params.headlampPower || 1.0).toFixed(1)}x</span>
            </div>
            <input
              type="range"
              min="0.0"
            ...
            <div className="flex justify-between items-center text-nano pt-2">
              <span className="text-neutral-200">Эфирный Туман (Volumetric Fog)</span>
              <span className="text-micro font-mono text-amber-400">{(params.volumetricFog || 0.4).toFixed(1)}x</span>
            </div>
            <input
              type="range"
              min="0.0"
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <div className="flex justify-between items-center text-nano">
              ...
              <span className="text-micro font-mono text-amber-400">{Math.round((params.interiorCut ?? 0.4) * 100)}%</span>
            </div>
            <input
              type="range"
              min="0.0"
            ...
            return (
              ...
              >
                <div className="text-left">
                  <span className={`text-nano font-semibold block ${isSelected ? 'text-amber-300' : 'text-neutral-200'}`}>
                    {pal.name}
                  </span>
                </div>
      ...
      {activeTab === 'gpu' && (
        <div className="space-y-3">
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <span className="text-nano font-semibold text-neutral-200 block">Целевая Частота Кадров (Target FPS)</span>
            <div className="grid grid-cols-4 gap-1.5">
              {[250, 165, 144, 120, 60, 0].map(fps => (
                <button
                  key={fps}
                  onClick={() => onParamsChange(prev => ({ ...prev, targetFps: fps }))}
                  className={`py-1.5 px-2 rounded-lg border text-center font-mono text-micro transition ${
            ...
            <p className="text-micro-sm text-neutral-400">Минимум 60 FPS, максимум 250 FPS на высокочастотных мониторах.</p>
          </div>

          {/* Dynamic Resolution Scaling (DRS) */}
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 flex items-center justify-between">
            <div>
              <span className="text-nano font-semibold text-neutral-200 block">Dynamic Resolution Scaling (DRS)</span>
              <p className="text-micro-sm text-neutral-400">Автоматически балансирует масштаб рендера для стабильного фреймрейта.</p>
            </div>
            <button
              onClick={() => onParamsChange(prev => ({ ...prev, drsEnabled: !prev.drsEnabled }))}
              className={`px-3 py-1.5 rounded-lg text-micro font-bold transition ${
                params.drsEnabled
          ...
          {/* Backend Selector */}
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <span className="text-nano font-semibold text-neutral-200 block">Вычислительный Конвейер</span>
            <div className="grid grid-cols-3 gap-1.5">
              {[
                ...
                <button
                  ...
                  className={`py-1.5 px-2 rounded-lg border text-center text-micro transition ${
                    forcedBackend === b.id
                      ? 'bg-amber-400 text-neutral-950 font-bold border-amber-300'
                      : 'bg-neutral-900 border-neutral-800 text-neutral-300 hover:border-neutral-700'
                  }`}
            ...
            <div className="text-micro-sm text-neutral-400 flex items-center justify-between pt-1 font-mono">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-nano font-semibold text-neutral-200 block">Акустический Резонатор φ</span>
                <p className="text-micro-sm text-neutral-400">Теплый гармонический дрон на частоте 432 Гц</p>
              </div>
            </div>
            <button
          ...
          {/* Tuning Mode Picker */}
          <div className="bg-neutral-900/60 p-3 rounded-xl border border-neutral-800 space-y-2">
            <span className="text-nano font-semibold text-neutral-200 block">Акустический Строй (Harmonic Mode)</span>
          ...
          <div className="p-2.5 rounded-xl bg-amber-950/25 border border-amber-500/30 text-micro-sm text-amber-300/90 leading-relaxed">
      ...
      {activeTab === 'neuro' && (
        <div className="space-y-3 bg-neutral-900/60 p-3.5 rounded-xl border border-neutral-800">
          <div className="flex items-center justify-between pb-2 border-b border-neutral-800">
            <span className="text-nano font-semibold text-neutral-200">Профиль Зрительной Коры</span>
            <span className="text-micro font-mono text-amber-400">
          ...
          <p className="text-micro-sm text-neutral-400 pt-2 border-t border-neutral-800 leading-relaxed">
