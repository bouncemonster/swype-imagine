...
- **Perf #5: Gate overlay components on loader dismissal** (`App.tsx`) — `FractalScrollFeed`, `FractalInfoHUD`, `FractalProbeHUD` now mount only after `loaderDismissed` (set by `CosmicLoader.onFinished`). Previously they did layout/render behind the full-screen overlay. *Est impact: −200–400 ms boot scripting+layout.*
- **Perf #4 (safe subset): `React.lazy()` `ControlsPanel`** (`App.tsx`) — the 45 KB engineer panel is now split into its own 33 KB chunk (`ControlsPanel-BWbFOSzo.js`), fetched on first `isEngineerMode=true`. Boot `index` chunk shrank **458 KB → 427 KB** (−31 KB, −6.8 %), so the *parse+compile* cost of that panel is off the critical path. Full dynamic-import of the 366 KB shader monolith was assessed and deferred — ShaderManager v3 needs the whole `FRAGMENT_SHADER_SOURCE` at init to splice the minimal per-fractal shader, so a boot-sequence or build-time change is prerequisite. Mobile-design-audit still passes (`panelOpen=true` on 375/430/667×375) because its `waitForFunction` on `#tab-gpu-btn` absorbs the chunk fetch.
- **Perf #3 (safe subset): `React.lazy()` the 4 always-mounted closed modals** (`App.tsx`) — `ExplanationModal` (16 KB), `UserProfileModal` (14 KB), `ProjectManifestModal` (7 KB), `FractalAtlasModal` (33 KB + 108 KB chunk including its transitive ~95 KB `canonicalFractals.ts` catalog) all `return null` when `isOpen=false` and have no mount-time side effects, so their parse+eval moves off the boot bundle entirely via `React.lazy()` + `<Suspense fallback={null}>`. Boot `index` chunk shrank **427 KB → 283 KB** (−144 KB, −33.7 %) on top of the #4 win, and cumulative from the pre-perf baseline **458 KB → 283 KB** (−38 %). Full `palettesProcedural` / `NeuroAestheticsEngine init` deferral (the *other* two Perf #3 sub-items) remains open — the boot path currently reads `ALL_COLOR_PALETTES[0]` synchronously for the first specimen, so a proper fix requires either an idle-time hook or a curated-61 palette slice.
- **Gap 4 from DESIGN.md: `theme-color` corrected** — `#0a0a0a` → `#030305` (matches the actual painted root background, eliminating mobile browser chrome band).

### Design system
- **Design-qa pipeline wired + glow ramp tokenized (DESIGN.md as acceptance contract)** — first full Design QA run against `DESIGN.md` via new `design.qa.yaml` (plugin `run-design-qa`: 3 masked screens 1280/390/768 — canvas excluded for nondeterminism; contract anchors `hud-trigger-pill`/`hud-icon-button` measured live; axe a11y **0 findings**; ui-alignment **Pass**; baselines in `.design-qa/expected/`). Sweep fixes: five *static* inline `boxShadow` glows → `@theme --shadow-glow-{dot,node,action,cta,modal}` utilities (`ControlsPanel`, `CosmicLoader`, `ProjectManifestModal`×2, `UserProfileModal`); `html, body` base literals → `var(--color-canvas)`/`var(--color-ink)` (new ink token); two redundant inline `touchAction` styles → `touch-none`/`overscroll-none` utilities (`FractalCanvas`). Test infra: `?test=1&pauseRender=1` harness flag (`useRenderEngine.ts`) auto-freezes the render loop after 3 frames — CDP screenshot capture went from 30.5 s (timeout-starved) to ~1 s. Debt audit hard-coded-color 3 → 1 (remainder: structural SVG `stopColor` var-styles). Zero visual delta re-proven: computed-style probe multiset IDENTICAL vs pre-migration baseline (1,225 nodes × 7 states). `DESIGN.md` reconciled: `hud-icon-button` height 32→30px measured + padding corrected, Elevation section documents the sanctioned glow ramp, Gap 2 follow-up added. Verified: `tsc` 0, build ok (CSS 81.84 → 83.46 kB — v4 shadow-utility composition), `npm run test` 1875/0, `test:mobile` PASS (tap<44=0, text<12=0, overflowX=0, desktop control untouched), `test:quality` pass.
- **DESIGN.md Gap 1 fully migrated: 173 arbitrary text sizes + 2 arbitrary surfaces → named `@theme` utilities** (`src/index.css`, 10 components) — every `text-[8px]`/`text-[9px]`/`text-[10px]`/`text-[11px]` call site in `ControlsPanel` (78), `UserProfileModal` (21), `ExplanationModal` (18), `TelemetryHUD` (13), `FractalInfoHUD` (11), `FractalAtlasModal` (10), `FractalProbeHUD` (8), `FractalScrollFeed` (8), `ProjectManifestModal` (4), `CosmicLoader` (2) became `text-micro-xs` / `text-micro-sm` / `text-micro` / `text-nano`; `bg-[#06050b]` → `bg-surface-loader`, `bg-[#090812]` → `bg-surface-modal`. `src/**` now has **zero** `text-[8-11px]` and zero `bg-[#hex]` sites. The micro tiers deliberately dropped their `--text-*--line-height` partners: `text-[Npx]` emits font-size only, so an lh-carrying token would have shifted every migrated HUD line. **Zero visual delta proven** by a computed-style multiset diff (`tests/css-equiv-probe.mjs`): 1,225 text nodes across 7 UI states (loader/HUD/profile/controls/atlas/explanation/manifest) report byte-identical `(fontSize, lineHeight)` pair counts pre- vs post-migration, across two runs each. Verified: `tsc` 0, build ok (boot `index` 282.9 KB, CSS 82.19 → 81.84 KB), `npm run test` 1875/0, `test:quality` pass, `npm run test:mobile` PASS (tap<44=0, text<12=0, overflowX=0, safeRules=5 — the coarse floor covers the new class names). `docs/DESIGN_DEBT.md` Bucket 2 closed (its "25 sites" was an undercount), `docs/index-css.md` migration-policy section replaced with completion status.
- **DESIGN.md Known Gap 5 closed: global `prefers-reduced-motion` CSS opt-out** (`src/index.css`) — before this, `prefers-reduced-motion` was only honoured at boot in `App.tsx` (render loop); the 12 `animate-pulse`/`animate-ping` sites plus the 2 `animate-[spin_…]` usages kept looping for reduced-motion users. A `@media (prefers-reduced-motion: reduce)` block now sets `animation-duration: 0.001ms`, `animation-iteration-count: 1`, `transition-duration: 0.001ms`, `scroll-behavior: auto` for `*, *::before, *::after`. Fractal morph/rotation (which is content, not decoration) remains governed by the pre-existing `PREFERS_REDUCED_MOTION` module-level check in `App.tsx`.
- **DESIGN.md Known Gap 2 closed: hard-coded amber hexes → tokens** — all 14 `#f59e0b` / `#fbbf24` / `#fef08a` / `#fde68a` / `#fef3c7` / `#b45309` literals and `rgba(245,158,11,…)` shadow strings in `CosmicLoader`, `ControlsPanel`, `ProjectManifestModal`, `UserProfileModal` migrated to `var(--brand-amber)` / `var(--brand-amber-bright)` / `var(--brand-amber-soft)` / `var(--brand-amber-warm)` / `var(--brand-amber-deep)` aliases (declared in `src/index.css`, forwarding to Tailwind v4's `--color-amber-{500,400,200,300,600}` oklch tokens). SVG `<stop stopColor>` uses `style={{stopColor:'var(...)'}}`; SVG `<path stroke>` and `<circle fill>` inherit `currentColor` from the parent's `text-amber-*` utility. `shadow-[…rgba(245,158,11,…)]` arbitrary utilities became inline `style={{boxShadow:'0 0 Npx color-mix(in srgb, var(--brand-amber) X%, transparent)'}}` — Tailwind's arbitrary-value parser doesn't cleanly handle `color-mix()`'s inner spaces. Fixes the measured Δ11 / Δ36 v3-vs-v4 hue drift; `docs/DESIGN_DEBT.md` Bucket 1 marked RESOLVED with `<details>`-wrapped original text preserved for context. Verified: `grep` 0 remaining amber hexes in `src/components/*.tsx`, `tsc` 0, mobile-design-audit PASS, `npm run test` 1875 passed.
- **DESIGN.md Known Gap 1 closed: `@theme` token layer** (`src/index.css`) — additive Tailwind v4 `@theme` block declares micro text tiers (`text-micro` 10px, `text-nano` 11px, `text-micro-sm` 9px, `text-micro-xs` 8px), semantic surfaces (`bg-canvas` #030305, `bg-surface-deep` #0a0a0a, `bg-surface-loader` #06050b, `bg-surface-modal` #090812), and brand aliases (`text-brand`, `bg-brand-bright`, etc. → `--color-amber-*`). Creates 14 named utilities replacing 173+ arbitrary values going forward. All existing code compiles unchanged (additive). Coarse-pointer floor rule extended to cover new `.text-micro`/`.text-nano` class names alongside legacy `text-[8-11px]`. Zero visual delta; `tsc` 0, build ok, tests 1875/0.
- **DESIGN.md Gap 4 closed: `theme-color` corrected** — `index.html` `#0a0a0a` → `#030305` (matches root background), mobile browser chrome no longer bands.
- **DESIGN.md Gap 6 closed: micro-type tier documented as desktop-only** — the `@theme` micro tiers are explicitly a desktop/fine-pointer density decision; coarse-pointer devices automatically receive the 12px floor via `@media (pointer: coarse)`. No code change, documentation closure.
...
- **`DESIGN.md` (repo root) — full design-system contract** captured via the marketplace `design-system-capture` skill (read-only inventory + write mode authorized by user). 437 lines: machine-readable YAML frontmatter (18 colour tokens, 9 typography levels, 6 radius tiers, 8 spacing steps, 14 component entries including state variants) plus prose body (Overview, Colors, Typography, Layout, Elevation, Shapes, Components, Do’s/Don’ts, Responsive Behavior, Known Gaps ×6, Evidence). Token values verified against `node_modules/tailwindcss/theme.css` and measured in the live browser (oklch→sRGB pixel readback); usage counts from `Select-String` across 3016 lines of TSX. `design-md-review` linter: 0 findings, ok=true.
- **CI `mobile-design-audit` job** (`.github/workflows/ci.yml`): a second job that builds the production bundle, installs Playwright Chromium, starts a local `vite preview` on 127.0.0.1:4173 (never depends on external network/VPN), runs `npm run test:mobile -- http://127.0.0.1:4173`, then stops the server. Initially `continue-on-error: true` (flip to blocking after first green Actions run). Validated with `npx js-yaml` (exit 0).
