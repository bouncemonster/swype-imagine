...
13. **Exposure + internal evolution parity**: the PBR composite was rebased into ACES' chromatic region (`diffuse 0.95 / bounce 0.55 / spec 0.7 / rim 0.6 / sss 0.7`, AO `0.4+0.6*ao`) and `sceneSDF` now drifts `phi` on golden sub-harmonics of `t` + breathes `iters` ±2, mirroring the WebGL fixes. Post-processing also matches WebGL: bloom threshold 0.6→0.8 / strength 0.35→0.16, the noisy time-varying CA/dither hashes were replaced with a static radial CA lift + static dither, and the raymarch start-jitter `u_time` term was dropped (edge flicker). A **boundary fade** (`col *= 1.0 - smoothstep(3.5, 5.0, length(p))`) was added to dissolve the hard `r_bound>5.0` sphere clip on space-filling fractals. WGSL never had the dead DOF/motion-blur loops. **The 10 render styles (solid…gemstone plus wireframe/heatmap/neon) are fully mirrored to WGSL for cross-engine parity**: a shared `relief` carrier (`clamp(dot(col, luma)*1.7, 0.22, 1.35)`) plus a `baseCol` sliver keep the 3D form reading through every style (previously each style flattened `col` to a featureless silhouette); `fwidth()`-derived widths anti-alias the topo contours, holo scanlines and hex grid (fade instead of moire); and the holo glitch/CA/shimmer + quantum waves/flux + iridescent interference-order `u_time` terms were slowed or made static to kill per-frame flicker. Each style now ends in a `mix(baseCol * k, styleCol * relief, ...)` blend identical to WebGL.
...
15. **`morphGate` aliveness parity**: WGSL `sceneSDF` now mirrors the WebGL single-`morph_speed` aliveness model — `let morphGate = clamp(u.morph_speed,0,1)` gates the FRACTAL BREATHING amplitude and the ORBITAL PRECESSION rate (previously both ran off raw `u.time`, so the rigid wobble drowned the mathematical evolution and `morph→0` did not actually freeze the figure). The `phi`-fold sub-harmonics were raised to match WebGL (0.35/0.21→0.5/0.3, iter 0.13→0.18). Breath was also rebalanced to WebGL parity: it previously ran at half rate / half amplitude (`u.time*0.4` × `0.025`) vs WebGL's `u.time*0.8` × `0.06`, so the two engines breathed differently — now identical.
16. **PALETTE DETAIL LENS + auto-exposure parity (WGSL mirror)**: the post-dispatch block after the 10-style `if/else` (skipped for solid via `u.render_style > 0.5`) now mirrors the WebGL lens exactly — it re-tints each mode with the **true palette hue** (`let paletteRef = u.primary_color*0.55 + u.secondary_color*0.30 + u.accent_color*0.15; palHue = paletteRef/max(dot(paletteRef,luma3),1e-3)`), keeps a 0.22 trace of the mode's own hue, modulates by a Reinhard-bounded structural signal (`lumN = styleLum/(1+styleLum)`) × `relief`, adds the `micro = length(fwidth(n))` fine-crease and `curvNorm` structure terms, and ends in the HARD ANTI-BLIND CEILING (`col = col * min(1.0, 0.82/max(outLum,1e-3))`). It replaced the earlier ANTI-FLATTEN block, which derived hue from `baseCol` and therefore collapsed every palette to the base PBR's cream wash. **Auto-exposure parity was also closed**: WGSL previously fed `col` straight into `acesToneMap` with no highlight limit (WebGL had an `exposureTarget` block, WebGPU did not — a real cross-engine gap), so blown faces desaturated to white on WebGPU; a matching `let preMax = max(col.r,max(col.g,col.b)); let exposureTarget: f32 = 0.9; if (preMax > exposureTarget) { col = col * (exposureTarget/preMax); }` now precedes `acesToneMap`, and WebGL's target was lowered `1.1 → 0.9` in lockstep. The breath amplitude stays `0.11` in all three `sceneSDF` copies.
