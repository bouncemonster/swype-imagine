...
16. **Shared ANTI-FLATTEN reinforcement (WGSL mirror)**: a post-dispatch block after the 10-style `if/else` (skipped for solid via `u.render_style > 0.5`) re-injects `form` (`clamp(dot(baseCol, vec3<f32>(0.299,0.587,0.114))*1.35, 0.45, 1.5)` → `col = col * mix(1.0, form, 0.6)`), `micro = clamp(length(fwidth(n))*5.0, 0, 1)` accent term, and a `curvNorm` structure term — guaranteeing no stylized mode flattens the figure on either engine. The breath amplitude was raised `0.06 → 0.11` in WGSL in lockstep with the WebGL monolith and `ShaderManager` generator.
