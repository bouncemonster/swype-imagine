...
- **Shared ANTI-FLATTEN reinforcement**: the per-style `relief`/`baseCol` treatment is necessary but not sufficient — a stylized mode can still wash out the large-scale form or hide the finest folds. A single block runs **after** the style dispatch (before the fog, so it reaches every spliced + monolith shader) and is skipped for solid (`u_render_style > 0.5`): it re-injects `form` (global relief from `baseCol` luminance → `col *= mix(1, form, 0.6)`), `micro = length(fwidth(n))` (finest screen-space normal detail, tinted to the accent), and a `curvNorm` structure term — so from the smallest crease to the whole figure, outside and inside, geometry survives any style. WGSL mirror in `webgpuShaders.ts`.
