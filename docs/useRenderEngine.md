...
- Loading progress (`loadProgress` state, line ~79): set to 0.12 on context acquire (~lines 243/279); `glEngine.onCompileProgress` maps shader stages to `0.12 + (pct/100) * 0.8` (~line 280); `setLoadProgress(1)` + `onEngineReady` fire ONLY on the FIRST RENDERED FRAME via `firstRenderDoneRef` (~lines 519-527)
