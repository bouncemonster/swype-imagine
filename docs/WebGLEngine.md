...
- `lazyCompileShader(fractalIdx)` - Requests a minimal per-fractal program from ShaderManager; on success switches program, sets up VAO, caches uniform locations, and calls `beginWarmup()`.
...
- `render(timeSec, params)` - Frame rendering (skips while lazy compilation is in progress; `isSwappingShader` guard prevents re-entrant render)
...
- **LRU cache**: `maxCacheSize = 8` compiled programs; least-recently-used program is deleted on overflow (`cacheShader`). `clearCache(exceptFractalIndex?)` and `getCacheStats()` are exposed.
