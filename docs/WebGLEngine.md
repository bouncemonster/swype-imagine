...
- `canPrefetch` / `prefetchFractal(fractalIdx)` - Background pre-compile of predicted-next fractal shaders into the ShaderManager LRU cache. Safe whenever the GL context + ShaderManager are up and no swap is in flight (`canPrefetch`); the flag itself does NOT gate on the first frame, but the caller (`useRenderEngine`) now holds prefetch until the first real frame is painted, because ANGLE/D3D11 serializes the driver-side LINK step and overlapping cold-start figures 1–2 with the critical-path figure 0 delays first paint. Up to `PREFETCH_CONCURRENCY = 2` prefetches run in parallel (ANGLE compiles on its own thread pool via KHR_parallel_shader_compile) once that gate clears. Never touches the active program — the user keeps viewing the current fractal.
- `pollCompletion()` - Non-blocking `KHR_parallel_shader_compile` COMPLETION_STATUS_KHR poll: up to 16000 spins with `setTimeout(5ms)` yields and a single `gl.flush()` kickoff (never per-spin). The budget must exceed worst-case cold ANGLE link time (tens of seconds on real hardware) — exhausting it falls through to the BLOCKING status read, which freezes the whole tab (measured 3.4s main-thread stall before the budget raise; ShaderManager's `waitUntilCompiled` uses the same 16000-spin budget).
- `getSwapProgress()` - Real 0-100 compile progress of the swap the user is waiting on, from per-index ShaderManager stage reports (`compilePctByIndex`) — drives the "Initializing GPU… N%" chip instead of a blind pulse.
...
- `render(timeSec, params)` - Frame rendering. On a fractal-type change: if the program is already in the LRU cache (prefetched), `adoptCachedProgram()` swaps it SYNCHRONOUSLY — the very next draw call shows the new fractal, no swap guard, no loading overlay. Otherwise the async lazy-compile path runs with the `isSwappingShader` guard (prevents re-entrant render; TIME-based 120 s safety reset — the old 301-frame/5s budget fired mid-compile on real hardware, spamming "Shader swap timeout" and flickering the loading chip). Returns `true` only when `gl.drawArrays` actually executed.
