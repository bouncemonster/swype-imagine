...
- `canPrefetch` / `prefetchFractal(fractalIdx)` - Background pre-compile of predicted-next fractal shaders into the ShaderManager LRU cache. Safe whenever the GL context + ShaderManager are up and no swap is in flight (`canPrefetch`); it does NOT wait for the first frame, so on cold start figures 1 and 2 compile concurrently with figure 0. Up to `PREFETCH_CONCURRENCY = 2` prefetches run in parallel (ANGLE compiles on its own thread pool via KHR_parallel_shader_compile). Never touches the active program — the user keeps viewing the current fractal.
